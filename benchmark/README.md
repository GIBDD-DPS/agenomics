...
## Evidence Quality (v0.6.1)

Отдельный модуль `sensitivity.py` — не новые метрики методологии, а
проверка устойчивости уже существующих результатов:

- **Weight Sensitivity** — насколько меняется Trust Score при малых
  сдвигах весов (±1%/±3%/±5%/±10%). Нашла реальную особенность: ось
  `accountability` на порядок чувствительнее остальных из-за
  взаимодействия с потолком автономности.
- **Threshold Sensitivity** — устойчивость обнаружения деградации к
  выбору `mild_threshold` в `DriftMonitorV2`; слишком мягкий порог
  (0.10) пропускает mild-деградацию полностью.
- **Bootstrap 95% CI** — доверительный интервал Compatibility Accuracy
  v2 по 1000 пересемплированиям. Честная оговорка: интервал считается
  относительно вариативности СИНТЕТИЧЕСКОГО набора, не относительно
  реального мира.

Числа — в [`BENCHMARKS.md`](BENCHMARKS.md).
