...
### Добавлено
- **`docs/CONNECT_YOUR_AGENTS.md`** — практический гайд «как подключить Agenomics к реальным агентам за 15 минут». Закрывает последний барьер до predictive validity: не код, а отсутствие реальных production-наблюдений. Включает протестированный сквозной пример (не псевдокод) — паттерн «одно наблюдение на оценочный период, инциденты прикрепляются к тому же наблюдению»
